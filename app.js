...
// Every list that mixes expenses and income sorts newest-first:
//
//   1. the transaction date  — this is what the day headings group by
//   2. how recently the row was entered, income and expense treated alike
//
// The old key was `date + arrayIndex`. Expenses and income are two separate
// arrays that both start at index 0, so an expense sitting at index 40 always
// beat an income at index 2 on the same day, no matter which one was actually
// added later. That type bias is the bug this replaces.
//
// Step 2 is an estimate, not a fact: `expenses` and `income` have no creation
// timestamp, and their `row_id` sequences are independent — expenses row_id
// 480 and income row_id 20 can be the same afternoon, so raw row_id cannot be
// compared across the two tables. What IS comparable is how many rows landed
// in the same table afterwards: the newest row in either table has zero rows
// after it, the one before it has one, and so on. Ranking by that gap puts the
// genuinely-latest entries on top regardless of type.

function txnDayKey(row) {
  return parseSheetDate(row['Date'] || row['date'] || '');
}

// Cached per array instance. appData.expenses / appData.income are replaced
// wholesale on every load, so the cache invalidates itself.
const _maxRowIdCache = new WeakMap();
function maxRowId(rows) {
  if (!Array.isArray(rows) || !rows.length) return 0;
  const hit = _maxRowIdCache.get(rows);
  if (hit !== undefined) return hit;
  let max = 0;
  for (const r of rows) {
    const v = Number(r._rowIndex != null ? r._rowIndex : r.row_id) || 0;
    if (v > max) max = v;
  }
  _maxRowIdCache.set(rows, max);
  return max;
}

// Rows entered after this one, within its own table. Lower = more recent.
function txnGap(row, type) {
  const source = type === 'income' ? appData.income : appData.expenses;
  const id = Number(row._rowIndex != null ? row._rowIndex : row.row_id) || 0;
  return maxRowId(source) - id;
}

// Used automatically if a "Created At" column is ever added to these tables;
// harmlessly null until then.
function txnCreatedKey(row) {
  const raw = row['Created At'] || row['created_at'];
  if (!raw) return null;
...
function compareTxn(a, b) {
  const day = (b._day || 0) - (a._day || 0);
  if (day) return day;

  const ac = a._created, bc = b._created;
  if (ac != null && bc != null && ac !== bc) return bc - ac;
  if (ac != null && bc == null) return -1;
  if (ac == null && bc != null) return 1;

  return (a._gap || 0) - (b._gap || 0);   // fewer rows after it = entered later
}

// Single place that turns a raw expense/income row into the shape every list
// renders from, so the sort fields can never drift apart between screens.
function decorateTxn(row, type) {
  const isInc = type === 'income';
  return {
    ...row,
    _type: type,
    _amt: Number(row[isInc ? 'Income Amount' : 'Expense Amount'] || 0),
    _date: row['Date'],
    _cat: row['Category'],
    _desc: row['Description'],
    _pm: row['Payment Mode'],
    _rowIndex: row._rowIndex,
    _day: txnDayKey(row),
    _created: txnCreatedKey(row),
    _gap: txnGap(row, type),
  };
}
...
function resetAppState() {
  ...
  ['person-loans-overlay','loan-action-overlay','emi-action-overlay',
   ...
   'import-overlay','clear-overlay','catbudget-overlay','delacct-overlay',
   'cal-overlay','pick-overlay','text-overlay'].forEach(id => {
...
// Prefers whatever was used last for this type, but only if it's still switched
// on. Anything stale — a category turned off since — falls back to the first
// active one rather than silently staying selected.
function defaultCategoryFor(tab) {
  const key = tab === 'income' ? 'income' : tab === 'loan' ? 'loan' : 'expense';
  const list = getActiveCategories(key);
  if (!list.length) return '';
  const last = (prefs.lastCat || {})[key];
  return (last && list.includes(last)) ? last : list[0];
}

// Guards against a category being switched off while it's the current pick
function validateKpCategory() {
  if (!kp || kp.tab === 'loan') return;
  const list = getActiveCategories(kp.tab);
  if (!list.length) { kp.category = ''; return; }
  if (!kp.category || !list.includes(kp.category)) kp.category = list[0];
}

function rememberCategory(key, value) {
  if (!value) return;
  prefs.lastCat = prefs.lastCat || {};
  if (prefs.lastCat[key] === value) return;
  prefs.lastCat[key] = value;
  persistPrefs();
}
...
// ── Calendar sheet ──
// A built calendar rather than the native picker, so it matches the rest of
// the app instead of dropping the user into the OS date dialog.
let _calView = null;               // month currently on screen

function openDayPicker() {
  _calView = new Date(kp.date.getFullYear(), kp.date.getMonth(), 1);
  renderCalSheet();
  document.getElementById('cal-overlay').classList.add('open');
}

function calShift(dir) {
  _calView = new Date(_calView.getFullYear(), _calView.getMonth() + dir, 1);
  buzz(5);
  renderCalSheet();
}

function calQuick(daysAgo) {
  const d = new Date();
  d.setDate(d.getDate() - daysAgo);
  pickCalDate(d.getTime());
}

function pickCalDate(ts) {
  kp.date = new Date(ts);
  buzz(8);
  renderDayStrip();
  closeOverlay('cal-overlay');
}

function renderCalSheet() {
  const MON = ['January','February','March','April','May','June','July',
               'August','September','October','November','December'];
  const y = _calView.getFullYear(), m = _calView.getMonth();
  document.getElementById('cal-month').textContent = `${MON[m]} ${y}`;

  const first = new Date(y, m, 1);
  const lead  = (first.getDay() + 6) % 7;          // Monday-first
  const total = new Date(y, m + 1, 0).getDate();
  const today = startOfDay(new Date()).getTime();
  const sel   = startOfDay(kp.date).getTime();
  ...
  for (let d = 1; d <= total; d++) {
    ...
    html += `<button class="cal-cell ${cls}" onclick="pickCalDate(${ts})">${d}</button>`;
  }
...
function renderKpChips() {
  ...
  if (kp.tab === 'loan') {
    ...
    html += chip(tag, kp.category || 'Pick a category', "openPicker('category')", !kp.category);
...
function choosePick(value) {
  if (_pickKind === 'category') { kp.category = value; rememberCategory(kp.tab, value); }
...
async function kpSubmit() {
  ...
  if (kp.tab !== 'loan') {
    validateKpCategory();
    if (!kp.category) {
      authError('kp-err', getActiveCategories(kp.tab).length
        ? 'Pick a category'
        : 'No categories switched on — add one in Settings');
      if (getActiveCategories(kp.tab).length) openPicker('category');
      return;
    }
  }
  ...
  if (kp.tab === 'expense') {
    ...
    // The API keys a due off `category` (Lent/Borrowed) and `description`
    // (who it's with) — the UI's friendlier names have to map back to those.
    params = { action:'addLoan', date, amount,
      category: kp.loanType,          // 'Lent' | 'Borrowed'
      description: kp.person,         // the other party
      paymentMode: kp.pm, remarks: kp.remarks || '-' };
...
function renderDashboard() {
  ...
  const allTxns = [
    ...expenses.map(r => decorateTxn(r, 'expense')),
    ...income.map(r   => decorateTxn(r, 'income')),
  ].sort(compareTxn).slice(0, 8);
...
function allTransactions() {
  ...
  return [
    ...appData.expenses.filter(acct).map(r => decorateTxn(r, 'expense')),
    ...appData.income.filter(acct).map(r   => decorateTxn(r, 'income')),
  ].sort(compareTxn);
}
...
function summaryRows() {
  ...
  return src
    .map(r => ({ ...decorateTxn(r, isExp ? 'expense' : 'income'),
      _amt: Number(r[amtKey] || 0), _cat: r['Category'] || 'Uncategorised' }))
    .filter(r => {
      const ts = parseSheetDate(r._date);
...
function openDay(year, month, day) {
  ...
  const rows = [
    ...accountRows(appData.expenses).map(r => decorateTxn(r, 'expense')),
    ...accountRows(appData.income).map(r   => decorateTxn(r, 'income')),
