...
async function refreshFromAPI() {
  try {
    ...
    if (!loadCachedData()) showToast('Failed to load data');
  }
}
...
function switchLaTab(tab) {
  document.querySelectorAll('#la-tabs .sheet-tab').forEach((t, i) =>
    t.classList.toggle('active', ['repay', 'history'][i] === tab));
...
function getActiveCategories(type) {
  // Start with defaults filtered by configState
  const defaults = type === 'expense' ? DEFAULT_EXPENSE : type === 'income' ? DEFAULT_INCOME : DEFAULT_LOAN;
  const state = configState[type];

  // Also check server-saved config
  const serverCfg = appData.config || {};
  const serverCustom = serverCfg[type + 'Custom'] ? serverCfg[type + 'Custom'].split(',').filter(Boolean) : [];
  const serverUnchecked = serverCfg[type + 'Unchecked'] ? serverCfg[type + 'Unchecked'].split(',').filter(Boolean) : [];
...
function renderDashboard() {
  ...
  const allTxns = [
    ...expenses.map((r, i) => ({ ...r, _type: 'expense', _amt: Number(r['Expense Amount'] || 0), _date: r['Date'], _cat: r['Category'], _desc: r['Description'], _pm: r['Payment Mode'], _rowIndex: r._rowIndex || i, _sortKey: sortKey(r, i) })),
    ...income.map((r, i)   => ({ ...r, _type: 'income',  _amt: Number(r['Income Amount']   || 0), _date: r['Date'], _cat: r['Category'], _desc: r['Description'], _pm: r['Payment Mode'], _rowIndex: r._rowIndex || i, _sortKey: sortKey(r, i) }))
...
function renderExpenses() {
  ...
  const rows = appData.expenses
    .map((r, i) => ({ ...r, _type: 'expense', _amt: Number(r['Expense Amount'] || 0), _date: r['Date'], _cat: r['Category'], _desc: r['Description'], _pm: r['Payment Mode'], _rowIndex: i, _sortKey: sortKey(r, i) }))
...
function renderIncome() {
  ...
  const rows = appData.income
    .map((r, i) => ({ ...r, _type: 'income', _amt: Number(r['Income Amount'] || 0), _date: r['Date'], _cat: r['Category'], _desc: r['Description'], _pm: r['Payment Mode'], _rowIndex: i, _sortKey: sortKey(r, i) }))
    .filter(r => { const ts = parseSheetDate(r._date); return ts >= range.from && ts <= range.to; })
    .sort((a, b) => b._sortKey - a._sortKey);
...
function renderLoans() {
  ...
  el.innerHTML = summary.map(l => loanCardHTML(l)).join('');
}
...
function loanCardHTML(l) {
  ...
  const bgClr     = isLent ? 'rgba(52,168,83,.12)' : 'rgba(234,67,53,.12)';
  const pending   = Number(l.pending || 0);
  const total     = Number(l.total   || 0);
  const paid      = total - pending;
  const pct       = total > 0 ? Math.round((paid / total) * 100) : 0;
  const typeLabel = isLent ? 'To Receive' : 'To Pay Back';
  const person    = l.person || 'Unknown';
  const loanId    = l.loanId || '';
  const initial   = person.charAt(0).toUpperCase();
  const avatarBg  = isLent ? 'rgba(52,168,83,.18)' : 'rgba(234,67,53,.18)';

  return `<div class="loan-card" onclick="openLoanAction('${loanId}')">
    <div class="loan-card-header">
      <div class="loan-card-left">
        <div class="loan-person-avatar" style="background:${avatarBg};color:${color}">${initial}</div>
        <div>
          <div class="loan-person-name">${person}</div>
          <div class="loan-person-meta">${loanId} · ${typeLabel}</div>
        </div>
      </div>
      <div class="loan-card-right">
        <div class="loan-amount" style="color:${color}">${fmt(pending)}</div>
