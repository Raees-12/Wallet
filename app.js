...
async function signOut() {
  if (currentUser) forgetSessionFor(currentUser.id);
  forgetLastUser();
...
async function switchAccount(id) {
  ...
  if (!stored || !stored.refresh_token) return askToSignInAs(a);
  ...
  try {
    ...
    // Refresh tokens rotate on use — store the replacement or the next
    // switch back to this account would fail.
    saveSessionFor(a.id, data.session);
    ...
    forgetSessionFor(a.id);          // it's no good any more
    askToSignInAs(a);
  }
}

// Fallback when there's no usable session for that account
function askToSignInAs(a) {
  showConfirm(
    `Sign in as ${a.username}?\n\nThat session has expired, so you'll need your password once more.`,
    async () => {
      const mail = a.email || '';
      await signOut();
