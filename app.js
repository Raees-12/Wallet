...
let _resendLeft = 0;
let _otpSentAt = 0;
let _signupPassword = '';         // set while a new account is being confirmed
...
function showAuthStep(step) {
  _authStep = step;
  ['welcome','form','otp','reset','loading'].forEach(k => {
  ...
  if (step === 'otp') setTimeout(() => document.getElementById('auth-otp').focus(), 140);
}
...
function setAuthMode(mode) {
  ...
  document.getElementById('auth-submit-btn').textContent = signup ? 'Create account' : 'Connect';
...
async function submitAuthForm() {
  ...
  try {
    ...
    if (_authMode === 'signup') {
      ...
      if (data.session) {
        // Confirmation is switched off — straight in
        await onSignedIn();
      } else {
        // Confirmation required: send a code and collect it here
        _pendingEmail = email;
        _signupPassword = pass;
        document.getElementById('auth-email-shown').textContent = email;
        document.getElementById('auth-otp').value = '';
        authError('auth-otp-err', '');
        authOK('auth-otp-ok', 'Check your inbox for the 6-digit code.');
        showAuthStep('otp');
...
async function sendOTP() {
  ...
  authError('auth-form-err', ''); authOK('auth-form-ok', '');
  if (!email)                { authError('auth-form-err', 'Enter your email address first'); return; }
  if (!EMAIL_RE.test(email)) { authError('auth-form-err', "That doesn't look like a valid email"); return; }
  ...
  try {
    ...
    const { error } = await sb.auth.signInWithOtp({
      email,
      options: {
        shouldCreateUser: true,               // same flow signs up and signs in
    ...
    document.getElementById('auth-otp').value = '';
    authError('auth-otp-err', '');
    authOK('auth-otp-ok', 'Code sent — check your inbox.');
    showAuthStep('otp');
    startResendCountdown(45);
  } catch(e) {
    authError('auth-form-err', friendlyAuthError(e));
...
function onOTPInput(el) {
  ...
  authError('auth-otp-err', '');
  if (el.value.length === 6) verifyOTP();      // auto-submit on the sixth digit
}

async function verifyOTP() {
  const code = document.getElementById('auth-otp').value.trim();
  authError('auth-otp-err', ''); authOK('auth-otp-ok', '');
  if (code.length !== 6) { authError('auth-otp-err', 'Enter the 6-digit code'); return; }

  const btn = document.getElementById('auth-verify-btn');
  btn.textContent = 'Verifying…'; btn.disabled = true;
  try {
    await ensureSupabase();
    // 'signup' confirms a brand-new account; 'email' covers everything else
    let { data, error } = await sb.auth.verifyOtp({
      email: _pendingEmail, token: code, type: _signupPassword ? 'signup' : 'email',
    });
    if (error && _signupPassword) {
      // Some projects report the code as an 'email' token instead — try both
      ({ data, error } = await sb.auth.verifyOtp({
        email: _pendingEmail, token: code, type: 'email',
      }));
    }
    if (error) throw error;
    if (!data || !data.session) throw new Error('Could not start a session');
    _signupPassword = '';
    stopResendCountdown();
    await onSignedIn();
  } catch(e) {
    authError('auth-otp-err', friendlyAuthError(e));
    document.getElementById('auth-otp').select();
  }
...
async function resendOTP() {
  ...
  try {
    await ensureSupabase();
    const { error } = _signupPassword
      ? (await sb.auth.resend({ type: 'signup', email: _pendingEmail }))
      : (await sb.auth.signInWithOtp({ email: _pendingEmail, options: { shouldCreateUser: false } }));
    if (error) throw error;
    _otpSentAt = Date.now();
    authOK('auth-otp-ok', 'New code sent.');
    startResendCountdown(60);                  // back off a little each time
  } catch(e) {
    authError('auth-otp-err', friendlyAuthError(e));
...
function startResendCountdown(seconds) {
  ...
  const tick = () => {
    ...
    btn.disabled = true;
    btn.textContent = `Resend code in ${_resendLeft}s`;
    _resendLeft--;
  };
  tick();
...
function backToEmail() {
  ...
  el.value = _pendingEmail;
  setTimeout(() => el.select(), 120);
}
...
async function signInWithGoogle() {
  ...
  try {
    ...
    // The SDK normally navigates away. If it hands back a URL without moving —
    // which happens inside installed PWAs and some in-app browsers — go there
    // ourselves rather than appearing to do nothing.
    if (data && data.url) {
    ...
    setTimeout(() => {
      ...
      authError('auth-form-err',
        "Google didn't open. If you're using the installed app, try signing in from your browser once.");
    }, 4000);
  } catch(e) {
    btn.disabled = false;
    btn.textContent = label;
...
async function signOut() {
  try { await ensureSupabase(); await sb.auth.signOut(); } catch(e) {}
  stopResendCountdown();
  clearCache();
  currentUser = null;
  appData = { expenses:[], income:[], loans:[], loanSummary:[], emis:[], emiPayments:[], accounts:[], config:{} };
  _pendingEmail = '';
  ['auth-otp','auth-email','auth-pass','auth-newpass'].forEach(id => {
    const el = document.getElementById(id); if (el) el.value = '';
  });
  ['auth-form-err','auth-otp-err','auth-reset-err'].forEach(id => authError(id, ''));
  authOK('auth-form-ok', ''); authOK('auth-otp-ok', '');
  setAuthMode('login');
  showAuthStep('welcome');
  showScreen('login-screen');
}
...
// Supabase keeps one session per browser, so "switching" means signing out and
// signing in as the other account rather than holding two sessions at once.
async function switchAccount(id) {
  ...
  showConfirm(
    `Sign in as ${a.username}?\n\nYou'll be signed out of this account first and sent a fresh code.`,
    async () => {
      _pendingEmail = a.email || '';
      await signOut();
      if (_pendingEmail) document.getElementById('auth-email').value = _pendingEmail;
      showAuthStep('form');
    }, 'Continue');
}
...
function startAddAccount() {
  ...
  closeSettings();
  showConfirm(
    "Sign in with a different account?\n\nYou'll be signed out of this one first.",
    async () => { await signOut(); showAuthStep('form'); },
    'Continue');
}
...
document.addEventListener('DOMContentLoaded', async () => {
  // Apply the last-used look immediately; the signed-in account's own
  // preferences replace it once we know who they are.
  ...
  showAuthStep('loading');
  document.getElementById('auth-loading-text').textContent = 'Checking your session…';
  ...
  // React to sign-in/out anywhere, including the Google redirect landing back
  // here and tokens refreshing in the background.
...
function doLogout() {
  closeSettings();
  if (currentUser) { clearCacheFor(currentUser.id); dropAccount(currentUser.id); }
  signOut();
}
...
function downloadSampleCSV() {
  ...
  showToast('Sample downloaded — fill it in and import');
}
...
async function doImport() {
  ...
  try {
    ...
    if (res.success) {
      ...
      showToast(`Imported ${n} transactions`);
    } else {
