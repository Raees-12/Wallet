...
document.addEventListener('DOMContentLoaded', () => {
  // Apply the last-used look immediately; initMainScreen swaps in this
  // account's own preferences once we know who is signed in.
...
async function refreshFromAPI() {
  try {
    ...
    applyServerSettings(res.settings);
    applyDefaultAccount();
...
function applyServerSettings(remote) {
  ...
  if (prefs.theme  && typeof applyTheme === 'function') applyTheme(prefs.theme, true);
  if (prefs.accent && typeof setAccent  === 'function') setAccent(prefs.accent, true);
  balanceHidden = prefs.hideBalance;
  updateEyeIcon();
  renderAll();
...
function persistPrefs() {
  ...
  _syncTimer = setTimeout(pushSettings, 700);
}

async function pushSettings() {
  if (!currentUser) return;
  try {
    const res = await apiPost({ action: 'saveSettings', userId: currentUser.id, settings: prefs });
