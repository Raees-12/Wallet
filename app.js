...
function showAuthStep(step) {
  _authStep = step;
  ['choose','email','otp','loading'].forEach(k => {
    const el = document.getElementById('auth-step-' + k);
    if (el) el.style.display = (k === step) ? 'block' : 'none';
  });
  if (step === 'email') setTimeout(() => document.getElementById('auth-email').focus(), 120);
  if (step === 'otp')   setTimeout(() => document.getElementById('auth-otp').focus(), 120);
}
...
function authOK(msg) {
  const el = document.getElementById('auth-otp-ok');
  if (el) el.textContent = msg || '';
}
...
async function sendOTP() {
  ...
  authError('auth-email-err', '');
  if (!email)              { authError('auth-email-err', 'Enter your email address'); return; }
  if (!EMAIL_RE.test(email)) { authError('auth-email-err', "That doesn't look like a valid email"); return; }
  ...
  try {
    ...
    authOK('Code sent — check your inbox.');
    showAuthStep('otp');
    startResendCountdown(45);
  } catch(e) {
    authError('auth-email-err', friendlyAuthError(e));
  }
...
async function signInWithGoogle() {
  ...
  try {
    ...
    if (error) throw error;
    // The browser now navigates away to Google
  } catch(e) {
    btn.disabled = false;
    authError('auth-email-err', friendlyAuthError(e));
    showAuthStep('choose');
...
async function signOut() {
  try { await authReady; await sb.auth.signOut(); } catch(e) {}
  stopResendCountdown();
  clearCache();
  currentUser = null;
  appData = { expenses:[], income:[], loans:[], loanSummary:[], emis:[], emiPayments:[], accounts:[], config:{} };
  _pendingEmail = '';
  const otp = document.getElementById('auth-otp');   if (otp) otp.value = '';
  const em  = document.getElementById('auth-email'); if (em)  em.value = '';
  authError('auth-email-err', ''); authError('auth-otp-err', ''); authOK('');
  showAuthStep('choose');
  showScreen('login-screen');
}
...
async function switchAccount(id) {
  ...
  showConfirm(
    ...
    async () => {
      ...
      await signOut();
      if (_pendingEmail) {
        document.getElementById('auth-email').value = _pendingEmail;
        showAuthStep('email');
      }
    }, 'Continue');
}
...
function startAddAccount() {
  ...
  showConfirm(
    ...
    async () => { await signOut(); showAuthStep('email'); },
    'Continue');
}
