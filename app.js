...
async function switchAccount(id) {
  ...
  showToast('Switching to ' + a.username + '…');
  try {
    await ensureSupabase();
...
// ══════════════════════════════════════════════════════════════
// FAB ARC MENU
// The + fans out into the four kinds of entry rather than guessing which
// one the user wanted.
// ══════════════════════════════════════════════════════════════
let _fabOpen = false;
...
function openFabMenu() {
  ...
  document.getElementById('fab-menu').setAttribute('aria-hidden', 'false');
  document.getElementById('nav-fab').classList.add('open');
  _fabOpen = true;
  buzz(8);
  try { history.pushState({ walletFab: true }, ''); } catch(e) {}
}

function closeFabMenu(skipHistory) {
  if (!_fabOpen) return;
  _fabOpen = false;
  document.getElementById('fab-backdrop').classList.remove('open');
  document.getElementById('fab-menu').classList.remove('open');
  document.getElementById('fab-menu').setAttribute('aria-hidden', 'true');
  document.getElementById('nav-fab').classList.remove('open');
  if (!skipHistory) {
    try { if (history.state && history.state.walletFab) history.back(); } catch(e) {}
  }
}

// Menu closes first so the two transitions don't fight each other
function pickAdd(type) {
  closeFabMenu();
  setTimeout(() => {
    if (type === 'emi') openEMIAddModal();
    else openAddModal(type);
  }, 160);
}
...
function renderDayStrip() {
  ...
  const sel  = new Date(kp.date); sel.setHours(0,0,0,0);
  const today = new Date(); today.setHours(0,0,0,0);

  // Five days ending on the selected one, so today usually sits at the right
  const items = [];
  for (let i = 4; i >= 0; i--) {
  ...
  el.innerHTML = items.map(d => {
    ...
    return `<button class="kp-day ${on ? 'on' : ''}" onclick="kpSetDate(${d.getTime()})">
      <span class="kp-day-n">${d.getDate()}</span>
      <span class="kp-day-w">${isToday ? 'today' : days[d.getDay()]}</span>
    </button>`;
  }).join('');
}
...
function openDayPicker() {
  // Reuse the native picker rather than building another calendar
...
function openTextSheet(kind) {
  ...
  document.getElementById('text-title').textContent = isPerson ? 'Who is involved?' : 'Add a note';
  const inp = document.getElementById('text-input');
  inp.placeholder = isPerson ? 'Name' : 'What was it for?';
  inp.value = isPerson ? kp.person : kp.desc;
...
function saveTextSheet() {
  ...
  if (_textKind === 'person') kp.person = v; else kp.desc = v;
...
function openAddModal(forceTab) {
  // Falls back to the tab matching where the user is
  const tabMap = { loans: 'loan' };
  const tab = forceTab || tabMap[currentPage] ||
              (currentPage === 'summary' ? summaryType : 'expense');
  kp = kpDefaults(tab === 'emi' ? 'expense' : tab);
...
window.addEventListener('popstate', () => {
  if (_fabOpen) { closeFabMenu(true); return; }
